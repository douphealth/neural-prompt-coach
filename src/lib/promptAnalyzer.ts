...
function generateModelRewrites(prompt: string): ModelRewrite[] {
  const words = prompt.split(/\s+/);
  const topicWords = words.filter(w => w.length > 3 && !['write', 'create', 'make', 'about', 'that', 'this', 'with', 'from', 'have', 'will', 'would', 'could', 'should', 'please', 'help'].includes(w.toLowerCase()));
  const topic = topicWords.slice(0, 5).join(' ') || 'the requested topic';
  const cleanPrompt = prompt.trim().charAt(0).toUpperCase() + prompt.trim().slice(1);
...
- Formatting: Use standard markdown. Highlight crucial terms in **bold**.
- Volume: Aim for 800 - 1,200 words of extremely high information density.`
    },
    {
      model: 'claude',
...
<context>
The user is seeking expert guidance regarding: "${topic}".
</context>

<instructions>
Please perform the following core task:
"${cleanPrompt}"

To execute successfully, adhere to these guidelines:
1. Divide your response using explicit structural sections.
2. Focus on high conceptual depth. Avoid high-level generic statements; instead, offer specific mechanisms, edge cases, and root-cause analyses.
3. Incorporate at least 3 detailed, concrete examples to illustrate key arguments.
4. Output your response using clean, semantic Markdown.
</instructions>

<constraints>
- Never use filler phrases or repetitive preambles.
- If certain assumptions must be made, define them clearly inside an <assumptions> tag at the start of your response.
</constraints>`
    },
    {
      model: 'gemini',
...
## Prompt Guidelines
You are requested to fulfill the following task:
"${cleanPrompt}"

## Structured Output Parameters
To ensure maximum engagement and actionability:
1. **Interactive Summary**: Start with a 3-bullet point executive summary highlighting the core takeaways.
2. **Deep Dive**: Provide a comprehensive breakdown of the core components of "${topic}". Use bolding, emojis for readability, and explicit tables if presenting tabular comparison data.
3. **Action Checklist**: Create a step-by-step checklist that the user can immediately implement.
4. **Follow-up Questions**: Conclude with 3 interactive follow-up questions to help the user refine their thinking further.

## Tone & Constraints
- Tone: Collaborative, engaging, modern, and friendly yet professional.
- Guardrails: Focus on current industry standards. If a topic is subject to debate, present both perspectives objectively.`
    },
    {
      model: 'llama',
...
- DIRECTNESS: Do not include introductory text like "Sure, I can help with that" or "Here is the result." Start directly with the first section.
- NEGATIVE CONSTRAINT: Do NOT suggest generic strategies, high-level advice, or surface-level overviews. Every claim must be supported by practical examples.
- SPECIFICITY: Include exact figures, metrics, or realistic placeholders where appropriate.
- FORMAT: Use a clean H2/H3 header structure with bold keywords. Avoid long blocks of text; split ideas into concise paragraphs or bulleted lists.`
    }
  ];
}
...
export function analyzePrompt(prompt: string): AnalysisResult {
  ...
  return {
    ...
    modelRewrites: generateModelRewrites(prompt),
